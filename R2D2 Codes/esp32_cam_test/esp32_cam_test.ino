...
void handleStream() {
  ...
  float motion = detectMotion(fb->buf, fb->len);
...
void handleRoot() {
  ...
  html += "0deg,rgba(0,0,0,0.15),rgba(0,0,0,0.15) 1px,";
  html += "transparent 1px,transparent 2px";
  html += ");";
  ...
  html += "position:fixed;";
  html += "top:0;left:0;right:0;bottom:0;";
  html += "background:radial-gradient(ellipse at center,";
  html += "transparent 40%,rgba(0,20,40,0.8) 100%);";
  html += "pointer-events:none;";
  html += "z-index:999;";
  html += "}";

  // Holographic flicker animation
  html += "@keyframes hologramFlicker{";
  ...
  html += "<div class='status-item'><div>Status</div><div class='status-value'>LIVE</div></div>";
  html += "</div>";
